...
func (server *DefaultServer) Inquiry(ctx *gin.Context) {
	...
	logger.Info("Inquiry", zenlogger.ZenField{Key: "endpoint", Value: endpoint}, zenlogger.ZenField{Key: "queryParams", Value: ctx.Request.URL.Query()})
	...
	tsReq, err := assignService.AssignMiddlewareRequest("ts_adapter", "inquiry", &userProductConf, req)
	if err != nil {
		logger.Error(err.Error())
	...
	middlewResponseIDs, coreResponse, err := coreService.Request2("inquiry", &userProductConf, tsReq)
	if err != nil {
		logger.Error(err.Error())
	...
	logger.Debug("inquiry", zenlogger.ZenField{Key: "userConf", Value: userConf}, zenlogger.ZenField{Key: "middlewareResponseIDs", Value: middlewResponseIDs}, zenlogger.ZenField{Key: "coreResponse", Value: coreResponse})
