...
func (repo *DefaultTrxRepo) FindByTargetNumber(targetNumber string) (trx domain.Trx, errRes error) {
	...
	sqlStmt := `SELECT pid, amendment_date, source_code, source_merchant, target_product, target_number, bit61, amount, status, rc, rc_desc FROM trx WHERE deleted_at IS NULL AND source_code = 'INQUIRY' AND status = 'approve' AND created_at >= NOW() - INTERVAL 5 MINUTE AND target_number = ? LIMIT 1`
