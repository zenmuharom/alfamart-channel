...
func (repo *DefaultMiddlewareRequestRepo) FindAllByAttributes(middlewareRequest domain.MiddlewareRequest) (configs []domain.MiddlewareRequest, err error) {
	...
	sqlStmt := `SELECT id, field, type, middleware, condition_field_id, condition_operator, condition_value, parent_id, product_code, created_at, updated_at, activated_at, deleted_at FROM ( SELECT *, ROW_NUMBER() OVER ( PARTITION BY middleware, process, field, type ORDER BY CASE WHEN product_code <> 000000 THEN 0 ELSE 1 END, product_code DESC, id ) AS rn FROM middleware_request WHERE deleted_at IS NULL ) t WHERE t.rn = 1 AND middleware = ? AND process = ? AND (product_code = '000000' OR product_code = ?)`
